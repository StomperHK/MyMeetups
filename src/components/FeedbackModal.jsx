...
function FeedbackModal(props) {
  ...
  const modalRest = feedbackModalState.slice(1)

  useEffect(scheduleModalSelfClosing, [feedbackModalState, changeFeedbackModalState, modalRest])

  function scheduleModalSelfClosing() {
    if (feedbackModalState[0]) {
      ...
      timeoutToCloseModal.current =
        setTimeout(() => {changeFeedbackModalState([false, ...modalRest])}, duration)
    }
  }
  ...
  return (
    ...
    >
      <p>
        <span role="alert">{feedbackModalState[1]}</span>
      </p>
      <button onClick={() => changeFeedbackModalState([false, ...modalRest])}
        aria-hidden="true" tabIndex={getModalState() ? "0" : "-1"}
      >
